...
#' @param PercentOccurence Numeric. Column number in DietData containing the percent occurence data. If calculating an index that does not require frequency of occurence data, can be left as NA.
...
#' @details This function calculates compund indices (indices combining various diet measurements). Specifically, this function calculates three indices: The Index of Preponderance,
...
#' product of the frequency of occurence of prey with either their volumetric or weight contribution to the diet. The Index of Relative Importance (Pinkas et al., 1971) is calculated as the 
#' the product of the sum of the weight or volume of a prey item and the percent number and the percent frequency occurence. The Feeding Quotient (Hureau, 1970) is the product of the percent number
...
#' information on the the contribution of the various prey to the diet of the record required to calculate the indices. Note that the order of these columns does not matter as the frequency of occurence, 
#' precent number, and percent volume or weight are specified with the arguments PercentNumber, PercentOccurence, and PercentVolWeight respectively. Users should enter in the column number for these arguments 
...
#' by running data(Casaux1998). This example is for two unique diet records for the fish Harpagifer antarcticus from two different localities (Potter Cove and Harmony Point) and contains data on the frequency of occurence 
...
#' PercentOccurence = 3, PercentVolWeight = 5, ReturnRaw = TRUE, PercentOnly = FALSE)
...
CompositeIndices <- function (DietData, Indices = c("IOP","IRI","FQ"), PercentNumber = NA, PercentOccurence = NA, PercentVolWeight = NA, ReturnRaw = FALSE, PercentOnly = TRUE){
  ...
  for(record.index in seq_along(uni.records)){
    ...
    if(ReturnRaw == TRUE){
      #IndicesCalced <- data.frame(matrix(nrow = nrow(current.record),ncol = 1+length(Indices)+length(which(!is.na(c(PercentNumber,PercentOccurence,PercentVolWeight))==TRUE))))
      IndicesCalced <- data.frame(matrix(nrow = nrow(current.record),ncol = 1+length(which(!is.na(c(PercentNumber,PercentOccurence,PercentVolWeight))==TRUE))))
      RawCols <- c("PercentNumber","PercentOccurence","PercentVolWeight")
      Raw.Check <- which(!is.na(c(PercentNumber,PercentOccurence,PercentVolWeight)))
      ...
      if(any(colnames(IndicesCalced)=="PercentOccurence")){
        IndicesCalced$PercentOccurence <- current.record[,PercentOccurence]
      }
    ...
    if(any(Indices == "IOP")){
      Current.IOP <- Calc.IOP(DietData = current.record, PercentOccurence = PercentOccurence, PercentVolWeight = PercentVolWeight)
      if(PercentOnly == TRUE){
    ...
    if(any(Indices == "IRI")){
      Current.IRI <- Calc.IRI(DietData = current.record, PercentOccurence = PercentOccurence, PercentNumber = PercentNumber, PercentVolWeight = PercentVolWeight)
      if(PercentOnly == TRUE){
...
##################################
Calc.IOP <- function (DietData, PercentOccurence, PercentVolWeight){
  VO <- DietData[,PercentOccurence]*DietData[,PercentVolWeight]
  sumVO <- sum(VO)
...
##################################
Calc.IRI <- function (DietData, PercentNumber, PercentOccurence, PercentVolWeight){
  NVO <- (DietData[,PercentNumber]+DietData[,PercentVolWeight])*DietData[,PercentOccurence]
  sumNVO <- sum(NVO)
